...
c=a.Dom.query("#"+d.container_id).parent().attr("id");a.View.Scroll.create(c,{snap:"li"})}}}}(LUNGO);LUNGO.View.Scroll=function(a){var d={hScroll:false,vScroll:false,useTransition:true,momentum:true,lockDirection:true,fixedScrollbar:true,fadeScrollbar:true,hideScrollbar:true},e=function(b){return b.hasClass("horizontal")?true:false},c=function(b,f){a.Data.Cache.exists("scrolls")||a.Data.Cache.set("scrolls",{});var i=a.Data.Cache.get("scrolls");i[b]=new iScroll(b,f);a.Data.Cache.set("scrolls",i)};return{create:function(b,f){if(b){var i=a.Dom.query("#"+b);setTimeout(function(){var g=false,h=i[0];if(h.clientHeight<
h.scrollHeight){g=true;var j=h.scrollHeight+90;a.Dom.query(h).children().first().css("height",j+"px")}if(g){g=f;h=e(i)?"hScroll":"vScroll";g||(g={});g[h]=true;f=g=a.Core.mix(d,g);c(b,f)}},100)}else a.Core.log(3,"ERROR: Impossible to create a <scroll> without ID")},update:function(b,f){var i=a.Dom.query("#"+b),g=i.children().first();if(g.length===0){i.html('<div id="'+b+'_scrl"></div>');g=i.children().first()}g.html(f);a.View.Resize.scroll(i);!a.Data.Cache.get("scrolls",b)&&c(b);a.Data.Cache.get("scrolls",
b).refresh()},remove:function(b){if(a.Data.Cache.exists("scrolls")){a.Data.Cache.get("scrolls",b).destroy();a.Data.Cache.remove("scrolls",b)}},isHorizontal:e}}(LUNGO);LUNGO.View.Aside=function(a){return{toggle:function(d){a.Dom.query(d+" article").toggleClass("aside")}}}(LUNGO);LUNGO.View.Element=function(a){var d={BUBBLE:".bubble.count"};return{count:function(e,c){var b=a.Dom.query(e);if(b)if(c>0){var f=b.children(d.BUBBLE);if(f.length>0)f.html(c);else{f=LUNGO.Attributes.Data.Count.html.replace("{{value}}",c);b.append(f)}}else b.children(d.BUBBLE).remove()}}}(LUNGO);LUNGO.Dom=function(a,d){return{query:function(e){return d(e)}}}(LUNGO,Zepto);LUNGO.Dom.Event=function(a){var d=function(e,c,b){var f=false;switch(c){case "SWIPE":a.Dom.query(e).swipe(b);break;case "SWIPE_LEFT":a.Dom.query(e).swipeLeft(b);break;case "SWIPE_RIGHT":a.Dom.query(e).swipeRight(b);break;case "SWIPE_UP":a.Dom.query(e).swipeUp(b);break;case "SWIPE_DOWN":a.Dom.query(e).swipeDown(b);break;case "DOUBLE_TAP":a.Environment.isDesktop()?a.Dom.query(e).live(a.Events.get(c),b):a.Dom.query(e).doubleTap(b);break;default:f=true}return f};return{bind:function(e,c,b){d(e,c,b)&&
...
selector:"*",html:'<img src="{{value}}" class="icon" />'},Title:{tag:"title",selector:"header, footer, article",html:'<h1 class="title">{{value}}</h1>'},Back:{tag:"back",selector:"header, footer",html:'<a href="#back" data-target="section" class="back onleft button icon {{value}}"></a>'}};LUNGO.Data.Cache=function(a){var d={},e=function(b){return arguments.length===1?d[b]:d[arguments[0]][arguments[1]]},c=function(b){return d[b]?true:false};return{set:function(b,f){d[b]=c(b)?a.Core.mix(e(b),f):f},get:e,remove:function(b){if(arguments.length===1)delete d[b];else delete d[arguments[0]][arguments[1]]},exists:c}}(LUNGO);LUNGO.Data.Sql=function(a){var d={name:"lungo_db",version:"1.0",size:65536,schema:[]},e=null,c=function(g,h){a.Core.log(1,"lng.Data.Sql >> "+g);e.transaction(function(j){j.executeSql(g,[],function(k,l){f(h,l)},i)})},b=function(g,h){var j="",k;for(k in g)if(a.Core.isOwnProperty(g,k)){var l=g[k];if(j)j+=" "+h+" ";j+=k+"=";j+=isNaN(l)?'"'+l+'"':l}return j},f=function(g,h){a.Core.toType(g)==="function"&&setTimeout(g,100,h)},i=function(g,h){a.Core.log(3,"lng.Data.Sql >> "+h.code+": "+h.message)};return{init:function(g){d=
a.Core.mix(d,g);if(e=openDatabase(d.name,d.version,d.name,d.size)){g=d.schema;var h=g.length;if(h)for(var j=0;j<h;j++){var k=g[j];k.drop===true&&c("DROP TABLE IF EXISTS "+k.name);var l=k.name;k=k.fields;var m="",n=void 0;for(n in k)if(a.Core.isOwnProperty(k,n)){if(m)m+=", ";m+=n+" "+k[n]}c("CREATE TABLE IF NOT EXISTS "+l+" ("+m+");")}}else a.Core.log(3,"lng.Data.Sql >> Failed to connect to database.")},select:function(g,h,j){h=h?" WHERE "+b(h,"AND"):"";c("SELECT * FROM "+g+h,function(k){for(var l=
[],m=0,n=k.rows.length;m<n;m++)l.push(k.rows.item(m));f(j,l)})},insert:function(g,h){var j="",k="",l;for(l in h)if(a.Core.isOwnProperty(h,l)){var m=h[l];j+=j?", "+l:l;if(k)k+=", ";k+=isNaN(m)?'"'+m+'"':m}c("INSERT INTO "+g+" ("+j+") VALUES ("+k+")")},update:function(g,h,j){g="UPDATE "+g+" SET "+b(h,",");if(j)g+=" WHERE "+b(j,"AND");c(g)},drop:function(g,h){var j=h?" WHERE "+b(h,"AND"):"";c("DELETE FROM "+g+j+";")},execute:c}}(LUNGO);LUNGO.Data.Storage=function(){return{}}(LUNGO);LUNGO.Boot=function(a){return function(){a.Environment.start();a.Boot.Layout.start();a.Boot.Events.start();a.Boot.Data.start();a.Boot.Section.start();a.Boot.Article.start()}}(LUNGO);LUNGO.Boot.Layout=function(a){var d=null,e=null,c=function(){if(!location.hash||!d.addEventListener){d.scrollTo(0,1);var b=1,f=setInterval(function(){if(e.body){clearInterval(f);b="scrollTop"in e.body?e.body.scrollTop:1;d.scrollTo(0,b===1?0:1)}},15);d.addEventListener("load",function(){setTimeout(function(){d.scrollTo(0,b===1?0:1)},0)},false)}};return{start:function(){if(!a.Environment.isDesktop()){d=window;e=d.document;if(d.innerHeight==356){a.Dom.query("body").css("height","416px");c()}}}}}(LUNGO);LUNGO.Boot.Article=function(a){var d={LIST_IN_ARTICLE:"article.list",SCROLL_IN_ARTICLE:".scrollable",CHECKBOX_IN_ARTICLE:".checkbox, .radio"},e=function(i,g){for(var h=a.Dom.query(i),j=0,k=h.length;j<k;j++){var l=a.Dom.query(h[j]);a.Core.execute(g,l)}},c=function(i){if(i.children().length===0){var g=i.attr("id");i.append('<ul id="'+g+'_list"></ul>')}},b=function(i){i=i.attr("id");a.View.Scroll.create(i)},f=function(i){i.append("<span>&nbsp;</span>")};return{start:function(){e(d.LIST_IN_ARTICLE,c);
e(d.SCROLL_IN_ARTICLE,b);e(d.CHECKBOX_IN_ARTICLE,f)}}}(LUNGO);LUNGO.Boot.Data=function(a){return{start:function(){var d=a.Attributes.Data,e;for(e in d)if(a.Core.isOwnProperty(d,e))for(var c=d[e],b=a.Dom.query(c.selector),f=0,i=b.length;f<i;f++){var g=a.Dom.query(b[f]);a.View.Template.Binding.dataAttribute(g,c)}}}}(LUNGO);LUNGO.Boot.Events=function(a){var d=function(i){i.preventDefault()},e=function(){a.View.Resize.toolbars()},c=function(i){var g=a.Dom.query(this);g=f(g);a.View.Aside.toggle(g);i.preventDefault()},b=function(i){var g=a.Dom.query(this);switch(g.data("target")){case "section":var h=g.attr("href");h==="#back"?a.Router.back():a.Router.section(h);break;case "article":h=f(g);g=g.attr("href");a.Router.article(h,g);break;case "aside":h=f(g);a.View.Aside.toggle(h)}i.preventDefault()},f=function(i){return"#"+
i.parents("section").attr("id")};return{start:function(){a.Dom.Event.listener(document,"TOUCH_MOVE",d);a.Dom.Event.listener(window,"ORIENTATION_CHANGE",e);a.Dom.Event.live("aside a[href][data-target]","TOUCH_START",c);a.Dom.Event.live("a[href][data-target]","TAP",b)}}}(LUNGO);LUNGO.Boot.Section=function(a){var d={ARTICLE:"article",SECTION:"section"};return{start:function(){var e=a.Dom.query(d.SECTION),c=e.first(),b="#"+c.attr("id");c.addClass("current");a.Router.History.add(b);c=0;for(b=e.length;c<b;c++)a.Dom.query(e[c]).children(d.ARTICLE).first().addClass("current");a.Dom.query("aside").addClass("show");a.View.Resize.toolbars()}}}(LUNGO);
